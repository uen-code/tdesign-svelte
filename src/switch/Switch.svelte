<script>
  ...
  $: {
    ...
    if (isArray(label)) {
      const labelContent = value === activeValue ? label[0] : label[1];
  ...
  $: switchClass = {
    ...
    [STATUS.checked]: value === activeValue,
  };
  $: nodeClass = {
...
<div class={getClassString(switchClass)} {disabled} on:click={handleToggle}>
  ...
  <div class={getClassString(contentClass)}>
    {#if !loading}
      {#if $$slots.labelOpen && value === activeValue}
        <slot name="labelOpen" />
      {:else if $$slots.labelClose && value === inactiveValue}
