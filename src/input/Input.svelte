<script>
  ...
  // action
  function typeAction(node, renderType) {
...
<div class={getClassString(inputWrapClass)} {style}>
  <div class={getClassString(inputClass)} on:mouseenter={onInputMouseenter} on:mouseleave={onInputMouseleave}>
    ...
    {#if type === 'password'}
      <span class="{getClassString(inputSuffixClass)} {`${COMPONENT_NAME}__clear`}">
        {#if renderType === 'text'}
          <BrowseIcon class={`${COMPONENT_NAME}__suffix-clear`} on:click={emitPassword} />
        {:else}
          <BrowseOffIcon class={`${COMPONENT_NAME}__suffix-clear`} on:click={emitPassword} />
        {/if}
      </span>
    {/if}
    ...
    {#if showClear}
      <span class="{getClassString(inputSuffixClass)} {`${COMPONENT_NAME}__clear`}">
        <CloseCircleFilledIcon class={`${COMPONENT_NAME}__suffix-clear`} on:click={emitClear} />
      </span>
    {/if}
