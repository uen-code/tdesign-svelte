<script>
  ...
  $: isChecked = checkAll ? intersection(siftList, checkboxGroupValue).length === siftList.length : checked;
...
<label bind:this={labelNode} class={getClassString(checkboxClass)} tabindex={isDisabled ? undefined : '0'}>
  <input
    type="checkbox"
    tabindex="-1"
