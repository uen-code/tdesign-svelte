<script>
  ...
  $: colClass = {
    ...
    [`${COMPONENT_NAME}-push-${push}`]: push,
    [`${COMPONENT_NAME}-pull-${pull}`]: pull,
  };
...
<div class="{getClassString(colClass)} {getClassString(colSizeClass)}" style="{getStyleString(colStyle)}{style}">
  <slot />
</div>
